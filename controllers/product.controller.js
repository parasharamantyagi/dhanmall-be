...
const { objectFormat, arrayOfObject, int_toFixed, checkObj, merge_object, array_to_str } = require("../helpers");
...
exports.saveOrders = async (req, res, next) => {
  try {
    let inputData = objectFormat(req.body, ['contract_type', 'contract_number', 'type', 'pick', 'game_id']);
    ...
    invest_money = checkObj(inputData, 'type') && inputData.type === '2' ? invest_money * 9 : invest_money;
    ...
    saveOrderCalculation(merge_object({ order_id: array_to_str(order._id) }, inputData));
