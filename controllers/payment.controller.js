...
const { saveMyChildren, getMyChildren, updateMyChildren } = require("../models/my_childrens");
...
exports.addPayment = async (req, res, next) => {
  try {
    ...
    if(checkObj(resLavel1)){
      ...
      savePayment(merge_object(inputData,{user_id: resLavel1.user_id, type: 'reward_1'}));
    }
    if(checkObj(resLavel2)){
      ...
      savePayment(merge_object(inputData,{user_id: resLavel2.user_id, type: 'reward_1'}));
    }
    if(checkObj(resLavel3)){
      ...
      savePayment(merge_object(inputData,{user_id: resLavel3.user_id, type: 'reward_1'}));
    }
    if(checkObj(resLavel4)){
      let ammount = (setDataType(inputData.ammount) * USER_COMMISION.LAVEL_4) / 100;
      updateMyChildren(resLavel4._id,{first_reward: ammount});
      plusUserMoney(resLavel4.user_id, { money: ammount },'reward');
      savePayment(merge_object(inputData,{user_id: resLavel4.user_id, type: 'reward_1'}));
    }
